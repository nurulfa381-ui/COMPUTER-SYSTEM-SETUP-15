...
document.addEventListener("DOMContentLoaded", function(){
  try{
    currentProfile = C01K15Storage.requireProfile();
  }catch(error){
    return;
  }
        ...
        ${choices.map(choice=>`
          <label class="answer-option">
            <input
              type="radio"
              name="mcq_${index}"
              value="${encodeURIComponent(choice)}"
            >
            ${choice}
          </label>
        `).join("")}
...
function submitKT15(){
  ...
  if(!allAnswered()){
    ...
    byteFeedback("Masih ada bahagian yang belum dijawab.");
    return;
  }
  ...
  if(passed){
    ...
    if(!currentProfile.badges.includes("KT15-LULUS")){
      currentProfile.badges.push("KT15-LULUS");
    }
  }

  C01K15Storage.saveProfile(currentProfile);

  attemptNo.textContent = currentProfile.attempts[15] + 1;

  if(passed){
    showPending(record);
    byteFeedback("Tahniah. Markah anda melepasi 60 peratus dan sedang menunggu pengesahan Pegawai Penilai.");
  }else{
    showFail(record);
    byteFeedback("Markah masih belum mencapai 60 peratus. Ulang kaji KP15 dan cuba semula.");
  }
}
...
function byteFeedback(message){
  byteMessage.textContent = message;
}
